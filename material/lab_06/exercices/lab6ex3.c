...
// Définitions de registres de base pour l'accéléromètre ADXL345
...
// Registres pour le tap/double tap
...
#define ADXL345_TAP_AXES_X     0x01   // Active la détection sur l'axe X
#define ADXL345_TAP_AXES_Y     0x02   // Active la détection sur l'axe Y
#define ADXL345_TAP_AXES_Z     0x04   // Active la détection sur l'axe Z
#define ADXL345_INT_SINGLE_TAP 0x40   // Active la détection de tap simple (bit 6)
#define ADXL345_INT_DOUBLE_TAP 0x20   // Active la détection de double tap (bit 5)

#define DEVICE_NAME "adxl345"

// Carte des registres ADXL345 pour référence
// 0x00 - DEVID
// 0x1D - THRESH_TAP
// 0x21 - DUR
// 0x22 - LATENT
...
// Structure pour stocker les données privées du driver
struct adxl345_data {
    ...
    // État sysfs
    char tap_axis; // 'x', 'y' ou 'z'
    char tap_mode; // 0=désactivé, 1=simple, 2=double, 3=les deux
    atomic_t tap_count;
    struct mutex tap_lock; // protège tap_axis et tap_mode
    struct completion tap_event;
    int tap_wait_busy;
    int last_tap; // 1=simple, 2=double
};
...
static int adxl345_update_tap_axis(struct adxl345_data *data, char axis) {
    ...
    switch (axis) {
        case 'x': 
            axes = ADXL345_TAP_AXES_X; 
            break;
        case 'y': 
            axes = ADXL345_TAP_AXES_Y; 
            break;
        case 'z': 
            axes = ADXL345_TAP_AXES_Z; 
            break;
        default: 
        return -EINVAL;
    }
...
static int adxl345_update_tap_mode(struct adxl345_data *data, char mode)
{
    ...
    switch (mode) {
        case 0: 
            ...
            break;
        case 1: 
            enable = ADXL345_INT_SINGLE_TAP; 
            break;
        case 2: 
            enable = ADXL345_INT_DOUBLE_TAP; 
            break;
        case 3: 
            enable = ADXL345_INT_SINGLE_TAP | ADXL345_INT_DOUBLE_TAP; 
            break;
        default: 
            return -EINVAL;
    }
...
static ssize_t tap_axis_show(struct device *dev, struct device_attribute *attr, char *buf) {
    ...
    // Section critique : protège l'accès à tap_axis
...
static ssize_t tap_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    // Section critique : protège l'accès à tap_mode
    mutex_lock(&data->tap_lock);
    mode = data->tap_mode;
    mutex_unlock(&data->tap_lock);
    ...
    switch (mode) {
        case 0: 
            n = sysfs_emit(buf, "off\n"); 
            break;
        case 1: 
            n = sysfs_emit(buf, "single\n"); 
            break;
        case 2: 
            n = sysfs_emit(buf, "double\n"); 
            break;
        case 3: 
            n = sysfs_emit(buf, "both\n"); 
            break;
        default: 
            n = sysfs_emit(buf, "off\n");
    }
...
static ssize_t tap_wait_show(struct device *dev, struct device_attribute *attr, char *buf) {
    ...
    // Section critique : protège l'accès à tap_wait_busy et tap_event
    mutex_lock(&data->tap_lock);
    ...
    // Attend l'événement (le handler d'interruption fera complete)
...
// Handler d'interruption pour tap/double tap
static irqreturn_t adxl345_irq_thread(int irq, void *dev_id) {
    ...
    // Lit INT_SOURCE pour acquitter et déterminer la cause
    int_source = i2c_smbus_read_byte_data(client, ADXL345_INT_SOURCE_REG);
    if (int_source < 0)
        return IRQ_NONE;

    // Ne traite que les interruptions tap/double tap
    if (int_source & (ADXL345_INT_SINGLE_TAP | ADXL345_INT_DOUBLE_TAP)) {
        ...
        if (int_source & ADXL345_INT_SINGLE_TAP) {
            printk(KERN_INFO "adxl345: Tap simple détecté (status=0x%02x)\n", tap_status);
            tap_type = 1;
        }

        if (int_source & ADXL345_INT_DOUBLE_TAP) {
            printk(KERN_INFO "adxl345: Double tap détecté (status=0x%02x)\n", tap_status);
            tap_type = 2;
        }

        // sysfs : met à jour tap_count, last_tap, complete tap_event si besoin
...
static int adxl345_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    bool sysfs_axis = false, sysfs_mode = false, sysfs_wait = false, sysfs_count = false;
    ...
    while (1) {
        ...
        // Configure les registres de détection tap/double tap
        ...
        // Active la détection de tap sur l'axe Z
        ret = i2c_smbus_write_byte_data(client, ADXL345_TAP_AXES_REG, ADXL345_TAP_AXES_Z);
        if (ret) break;

        // Mappe toutes les interruptions sur INT1 (notamment TAP/DOUBLE TAP)
        ret = i2c_smbus_write_byte_data(client, ADXL345_INT_MAP_REG, 0x00);
        if (ret) break;

        // Active les interruptions tap simple et double
        ...
        // Demande une IRQ threadée pour la détection tap/double tap
        if (client->irq > 0) {
            ret = request_threaded_irq(client->irq, NULL, adxl345_irq_thread,
                                       IRQF_ONESHOT | IRQF_TRIGGER_RISING,
                                       DEVICE_NAME, data);
            if (ret) {
                dev_warn(&client->dev, "adxl345: Échec de la demande d'IRQ %d\n", client->irq);
            } else {
                data->irq_requested = 1;
                irq_registered = true;
                dev_info(&client->dev, "adxl345: IRQ %d enregistrée pour la détection de tap\n", client->irq);
            }
        } else {
            dev_warn(&client->dev, "adxl345: Pas d'IRQ configurée, détection tap désactivée\n");
        }

        // Enregistre les attributs sysfs
        ...
        dev_info(&client->dev, "adxl345: driver chargé\n");
        return 0;
    }
    ...
    if (irq_registered && client->irq > 0)
...
static void adxl345_remove(struct i2c_client *client) {
    ...
    // Supprime les attributs sysfs
    device_remove_file(&client->dev, &dev_attr_tap_axis);
    device_remove_file(&client->dev, &dev_attr_tap_mode);
    device_remove_file(&client->dev, &dev_attr_tap_wait);
    device_remove_file(&client->dev, &dev_attr_tap_count);

    // Désactive les interruptions tap
    ...
    // Libère l'IRQ si demandée
    if (data->irq_requested && client->irq > 0)
        free_irq(client->irq, data);

    dev_info(&client->dev, "adxl345: driver déchargé\n");
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("REDS");
MODULE_DESCRIPTION("Pilote caractère simple pour accéléromètre ADXL345");